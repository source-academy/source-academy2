...
function markTraitor(player) { // meant for Player subclassess
    if (player instanceof SecurityDrone)
        return;
    while(player.__proto__ instanceof Player) {
        player = player.__proto__;
    }
    var ghostDrone = new SecurityDrone();
    for (var droneStuff in ghostDrone.__proto__)
        delete ghostDrone.__proto__[droneStuff];
    SecurityDrone.__droneCount--;
    ghostDrone.__proto__.__proto__ = player.__proto__.__proto__; // Person
    player.__proto__.__proto__ = ghostDrone.__proto__;
}

Object.defineProperty(Weapon, "__atkReview", {value: function(attacker, targetArr, weapon) {
    if (weapon instanceof Bomb || attacker instanceof SecurityDrone) {
        return;
    }
    var attackedComrades = targetArr.filter(function (target) {return target instanceof Player && !(target instanceof SecurityDrone);});
    if ((weapon instanceof SpellWeapon && attackedComrades.length === targetArr.length) || attackedComrades.length > 0) {
        var comradeNames = "";
        for (i = 0; i < attackedComrades.length; ++i) {
            if (i === attackedComrades.length - 2) {
                comradeNames += " and ";
            }
            else if (i < attackedComrades.length - 2) {
                comradeNames += ", ";
            }
            comradeNames += attackedComrades[i].getName();
        }
        display_message(attacker.getName() + " has attacked " + comradeNames + "!");
        display_message(attacker.getName() + " declared rogue by JFDI Council!");
        Weapon.__statsMgrDelegate.__registerTraitor(attacker.getName());
        markTraitor(attacker);
    }
 }
});
